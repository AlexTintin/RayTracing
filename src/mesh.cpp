...
bool Mesh::intersectFace(const Ray& ray, Hit& hit, int faceId) const
{
    ...
    Vertex A(m_vertices[m_faces[faceId].x()]);
    Vertex B(m_vertices[m_faces[faceId].y()]);
    Vertex C(m_vertices[m_faces[faceId].z()]);

    Vector3f e1(A.position-C.position);
    Vector3f e2(B.position-C.position);

    Vector3f dir(ray.direction);
    Vector3f origine(ray.origin);
    Vector3f b(origine-C.position);
    ...
    if(t>Epsilon && u>=Epsilon && v>=Epsilon && (u+v)<=1 && t<hit.t())
    {
        hit.setT(t);
        ...
        Vector3f An(A.normal);
        Vector3f Bn(B.normal);
        Vector3f Cn(C.normal);
        hit.setNormal(u*An + v*Bn + (1.f-u-v)*Cn);

        Vector2f At(A.texcoord);
        Vector2f Bt(B.texcoord);
        Vector2f Ct(C.texcoord);
        hit.setTexcoord(u*At + v*Bt + (1.f-u-v)*Bt);
