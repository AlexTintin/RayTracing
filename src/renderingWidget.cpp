    ...
    for (int y=0; y<size.y(); ++y) {
        for (int x=0; x<size.x(); ++x) {
            ...
            for(int i=0; i<camera->sampleCount(); i++)
            {
                  ...
                  //But: Créer la procédure d'antialiasing, le nombre d'échantillon par pixel est camera->sampleCount()

                  float randx(x + offset[0]+(float)rand()/RAND_MAX);
                  float randy(y + offset[1]+(float)rand()/RAND_MAX);

                  ray.direction = (camF + camX * (2.0*float(randx)/float(camera->vpWidth())  - 1.)
                                                  - camY * (2.0*float(randy)/float(camera->vpHeight()) - 1.)).normalized();
